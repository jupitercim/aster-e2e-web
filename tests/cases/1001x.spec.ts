...
test.describe.serial('AsterDEX - 1001倍高杠杆交易', () => {
  ...
  // 测试 1：1001x 页面正常加载
  // ========================================================
  test('1001x 页面正常加载，显示杠杆滑动条和做多/做空按钮', { tag: ['@P0'] }, async ({ loggedInPage: page }) => {
  ...
  // ========================================================
  test('永续合约与预测模式 Tab 切换正常', { tag: ['@P0'] }, async ({ loggedInPage: page }) => {
  ...
  // ========================================================
  test('仓位、订单、历史 Tab 切换正常', { tag: ['@P0'] }, async ({ loggedInPage: page }) => {
