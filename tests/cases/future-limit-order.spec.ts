...
test.describe.serial('AsterDEX - 期货限价委托', () => {
  ...
  // 测试 1：限价挂单（买入/做多）
  // ========================================================
  test('限价挂单 BTC/USDT 0.001 BTC（mark price - 1000）', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
  ...
  // 测试 2：取消限价委托
  // ========================================================
  test('取消刚才的限价委托单', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
  ...
  test('验证历史委托中订单状态、价格与数量', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
  ...
  // 测试 4：「隐藏订单」选项可用
  // ========================================================
  test('验证隐藏订单 checkbox 可正常勾选', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
  ...
  // 测试 5：「只减仓」选项可用
  // ========================================================
  test('验证只减仓 checkbox 可正常勾选', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
  ...
  // ========================================================
  test('改单：修改价格成功，无变化时提示不用修改，最后取消所有订单', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
    await page.goto(process.env.EXCHANGE_URL!);
    await page.waitForLoadState('domcontentloaded');
    await page.waitForTimeout(1000);
