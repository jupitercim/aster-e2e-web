...
test.describe.serial('AsterDEX - Shield 模式交易', () => {
  ...
  // ========================================================
  test('Shield 模式页面可正常加载', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
  ...
  // 测试 2：Shield 模式限价买入
  // ========================================================
  test('Shield 模式限价买入 0.001 BTC', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
  ...
  // 测试 3：取消 Shield 委托单
  // ========================================================
  test('取消 Shield 委托单', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
  ...
  test.skip('网格交易 - 手动创建做多策略（价格区间 mark-2000 ~ mark-1000，5格）', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
    ...
    // ── 等待页面完全渲染（等 #gridLowerLimit 或 手动创建 出现）──
    await page.waitForSelector('#gridLowerLimit, [data-testid="manual"], button:has-text("手动创建")', { timeout: 20000 }).catch(() => {});
    ...
    await lowerInput.waitFor({ state: 'visible', timeout: 10000 }).catch(() => {});
  ...
  // ========================================================
  test('网格交易 - 查看策略详情并终止', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
    await page.goto(getGridUrl());
    await page.waitForLoadState('domcontentloaded');
