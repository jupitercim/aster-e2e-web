...
test.describe.serial('AsterDEX - 期货页面检查', () => {
  ...
  // 测试 6：最近成交
  // ========================================================
  test('最近成交', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
    // 复用已打开的页面
  ...
  // ========================================================
  test('底部面板 Tabs（持仓 & 委托）', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
    // 复用已打开的页面
  ...
  // 测试 9：充值 / 提现 / 转账 按钮与弹窗
  // ========================================================
  test('充值/提现/转账按钮与弹窗', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
    await page.goto(process.env.EXCHANGE_URL!);
    await page.waitForLoadState('domcontentloaded');
    await page.waitForTimeout(3000);
