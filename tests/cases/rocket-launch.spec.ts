...
test.describe.serial('AsterDEX - Rocket Launch 页面', () => {
  ...
  // 测试 1：页面加载，标题与主要文案可见
  // ========================================================
  test('Rocket Launch 页面可正常加载', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
    const url = getRocketLaunchUrl();
  ...
  // ========================================================
  test('Tab 切换：火箭发射 / Trade Arena', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
    // 验证两个 Tab 都可见
  ...
  // ========================================================
  test('筛选按钮切换正常', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
  ...
  // ========================================================
  test('项目卡片列表可见，交易赚取链接有效', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
    // 进入"已结束"筛选（卡片数量最多）
    const endedBtn = page.locator('button:has-text("已结束")').first();
    if (await endedBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await endedBtn.click();
  ...
  // ========================================================
  test('点击交易赚取进入 Spot 交易页', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
    const endedBtn = page.locator('button:has-text("已结束")').first();
    if (await endedBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await endedBtn.click();
      await page.waitForTimeout(1500);
  ...
  // ========================================================
  test('"查看更多"加载更多项目', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
    // 先进入"已结束"
    const endedBtn = page.locator('button:has-text("已结束")').first();
    if (await endedBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
      await endedBtn.click();
  ...
  // 测试 7：FAQ 手风琴展开/收起
  // ========================================================
  test('FAQ 手风琴展开与收起', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
    const faqs = [
      '什么是火箭发射？',
      '如何符合活动条件？',
      '我什么时候可以收到我的火箭发射计划奖励？',
  ...
  // ========================================================
  test('火箭发射 Tab - 已结束卡片：结束时间/最小持仓/交易赚取链接', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
  ...
  // ========================================================
  test('Trade Arena Tab - 已结束卡片：结束时间/最小持仓/交易赚取链接', { tag: ['@P0', '@PROD'] }, async ({ loggedInPage: page }) => {
