...
## 项目目标
本项目需要达成的目标分为短期目标（1-2months）、中期目标（6months）、长期目标（12months+），宗旨是提升aster项目的前端自动化测试覆盖范围和使用频率，增加测试效率达到最终提升项目质量的终极目的。

- 短期目标：搭建cc+playwright的自动化测试框架，分配自动化测试模块，基本覆盖发版的生产灰度回归测试。自动化覆盖率达到基础级。
- 中期目标：按分工模块由QA成员完善自动化测试用例的覆盖范围和精细化颗粒度，实现各个模块的前端功能全回归。并且确保能稳定地运行在QA环境、QA分支环境、生产灰度环境和正式生产环境，并能及时发现较隐蔽前端bug.自动化覆盖率达到核心流级。
- 长期目标：探索AI全自动化测试，从前端开发开始到部署QA，最终部署生产，直接由AI全自动进行功能测试+自动化脚本完成，QA成员仅需提供测试思路和方案完善。自动化覆盖率达到深度级。
...
| Future Limit Order | Limit Order / 隐藏订单 / 只减仓 | `future-limit-order.spec.ts` | andros  |
...
| Grid Trading | 网格交易策略 | `grid-trading.spec.ts` |  andros |
| 1001x | 高杠杆 1001x 交易 / 预测模式 | `1001x.spec.ts` |         |
| USDF | USDF 稳定币铸造 / 兑换 | `usdf.spec.ts` | Kevin.c |
...
| More Resources | More Resources | `more-resources.spec.ts` |  andors  |
| Rocket Launch | Rocket Launch 页面测试 | `rocket-launch.spec.ts` | andros |
| H5 Auto Test | H5 页面兼容测试 | `h5-auto.spec.ts` | andros  |
