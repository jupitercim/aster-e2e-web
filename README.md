...
- 短期目标：搭建cc+playwright的自动化测试框架，分配自动化测试模块，基本覆盖发版的生产灰度回归测试。
- 中期目标：按分工模块由QA成员完善自动化测试用例的覆盖范围和精细化颗粒度，实现各个模块的前端功能全回归。并且确保能稳定地运行在QA环境、QA分支环境、生产灰度环境和正式生产环境，并能及时发现较隐蔽前端bug.
- 长期目标：探索AI全自动化测试，从前端开发开始到部署QA，最终部署生产，直接由AI全自动进行功能测试+自动化脚本完成，QA成员仅需提供测试思路和方案完善。
